...
XHR.prototype.post3 = function(url, data, onSuccess, onError, extraParams) {
	...
	xhr.open("POST", url, extraParams.async);
	xhr.setTimeout(10000);
	if (Ti.App.Properties.getString('sessionValue') != null) {
		xhr.setRequestHeader('Cookie', '_session=' + Ti.App.Properties.getString('sessionValue'));
	} else {
		if (extraParams.shouldAuthenticate) {
			var authstr = 'Basic ' + Titanium.Utils.base64encode(extraParams.username + ':' + extraParams.password);
			xhr.setRequestHeader('Authorization', authstr);
	...
	/*var d1 = new Date();
	Ti.API.info('------ Current Time:' + d1);
	var d2 = new Date(d1);

	Ti.API.info('------ D2 BEFORE:' + d2);

	d2.setHours(d1.getHours() + 604800);
	Ti.API.info('------ D2: AFTER' + d2);

	xhr.setRequestHeader("Cookie", "OKAUTHLAST=" + d2);
	xhr.setRequestHeader("Cookie", "OKAUTHUSER=" + utility.Ui.getEMAIL());*/
